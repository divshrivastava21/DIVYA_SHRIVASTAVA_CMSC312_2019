...
public class MultiThreading3 extends Process implements Runnable {




    public MultiThreading3 (int input, File file, int memoryNeeded, int memoryNeededKB, int framesNeeded, int pagesNeeded,
        ...
        while (scan.hasNextLine()) {
            ...
            if (word.equals("CALCULATE") || word.equals("I/O")) {
                ...
                arr[i] = numInt + (int) (getRandomInt(numInt * (-0.8), numInt * (0.8)));
        ...
        processID[j] = pid;
        super.setPid(pid);
        arrivalTimeInReady[j] = 4; //Random.nextInt(input*4) + 1; //4;
        burstTimeCPU[j] = 1; /*(int)getRandomInt(1, 20);*/
    ...
    public class pipeThreading implements Runnable {
        ...
        public pipeThreading() {
            ...
            try {
                int l;
                if(getInput() == getCount()) {
                    while ((l = pipedInPut.read()) != -1) {
